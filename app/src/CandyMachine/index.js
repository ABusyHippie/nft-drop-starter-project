...
const CandyMachine = ({ walletAddress }) => {
  ...
  const [mintName, setName] = useState([]);
    ...
    if (data.length !== 0) {
      for (const mint of data) {
        ...
        if (!mints.find((mint) => mint === parse.image)) {
          ...
          setName((prevName) => [...prevName, parse.name]);
        }
      }
    }
  ...
  const renderMiningAnimation = () => (
    <div>
      <img alt="Mining Gif" className="mining-gif" src={miningGif} />
      <div>Mining your NFT now...</div>
    </div>
  );
  ...
  return (
    machineStats && (
      <div className="machine-container">
        ...
        {isMinting && renderMiningAnimation()}
